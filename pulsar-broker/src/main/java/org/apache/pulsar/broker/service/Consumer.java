...
public class Consumer {
     ...
     * <p>No-op if {@code pendingAcks} is not initialized.
     *
     * @return the removed remaining unacked count, or {@code -1} if not found
     */
    ...
    public void redeliverUnacknowledgedMessages(List<MessageIdData> messageIds) {
        ...
        for (MessageIdData msg : messageIds) {
            ...
            int pendingAck = pendingAcks.removeAndGetRemainingUnacked(position.getLedgerId(), position.getEntryId());
            if (pendingAck != PENDING_ACK_NOT_FOUND) {
                totalRedeliveryMessages += pendingAck;
